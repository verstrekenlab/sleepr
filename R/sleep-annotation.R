...
#' Score sleep behaviour from immobility using euclidean distance
#'
...
#' @import logging
#' @export
fsl_sleep_annotation <- function(data,
                             time_window_length = 10, #s
                             min_time_immobile = 300, #s = 5min
                             velocity_correction_coef = 3e-3, # 0.006
  ...
  wrapped <- function(d){
    ...
    d2 <- copy(d)
    d2[, distance := euclidean_distance(x, y)]
    d2[, xy_dist_log10x1000 := 1000*log10(distance)]
    d2[, distance := NULL]
    d_small <- motion_detector_FUN(d2, time_window_length, velocity_correction_coef, ...)
    rm(d2)
