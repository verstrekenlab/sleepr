...
test_that("sleep_annotation works return expected results", {
  ...
  d_small <- sleep_annotation(data=data, motion_detector_FUN = virtual_beam_cross_detector)
...
test_that("sleep_annotation works for single or multiple animals", {
  ...
  manual_multi_nanimal <- data[, sleep_annotation(data=.SD, motion_detector_FUN = virtual_beam_cross_detector), by="id"]

  auto_multi_animal <- sleep_annotation(data=data, motion_detector_FUN = virtual_beam_cross_detector)
